...
app.use(cors({
    origin: [
        ...
        'https://total1.onrender.com' // ADDED - This is your frontend URL
    ],
...
async function connectToMongoDB() {
    try {
        ...
        await casesCollection.createIndex({ status: 1 }); // ADDED for admin dashboard filtering
...
// ✅ IMPROVED: GET all cases with better error handling
...
// ✅ IMPROVED: CREATE case with proper caseId handling
app.post('/api/cases', async (req, res) => {
    try {
        console.log('📝 POST /api/cases request received:', req.body);
        
        // Use caseId from frontend or generate one
        const caseId = req.body.caseId || `CP-${new Date().getFullYear()}-${Math.floor(Math.random() * 9000 + 1000)}`;
        
        const caseData = {
            ...req.body,
            _id: generateId(),
            caseId: caseId, // Ensure caseId is set
            createdAt: new Date(),
            updatedAt: new Date()
        };
        ...
        const caseData = {
            ...req.body,
            _id: generateId(),
            caseId: req.body.caseId || `CP-${new Date().getFullYear()}-${Math.floor(Math.random() * 9000 + 1000)}`,
            createdAt: new Date(),
            updatedAt: new Date()
        };
        memoryStorage.cases.push(caseData);
...
app.put('/api/cases/:id', async (req, res) => {
    try {
        const id = req.params.id;
        console.log(`🔄 PUT /api/cases/${id} request received:`, req.body);
...
app.delete('/api/cases/:id', async (req, res) => {
    try {
        ...
        let result;
        if (useMongoDB && casesCollection) {
            result = await casesCollection.deleteOne({ _id: new ObjectId(id) });
        } else {
            const index = memoryStorage.cases.findIndex(c => c._id === id);
            if (index !== -1) {
